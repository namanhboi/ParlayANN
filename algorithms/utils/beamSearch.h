...
// main beam search
  // right now, it returns the visited nodes in order, not sorted by rank. This shouldn't affect anything.
...
std::pair<
	  std::pair<
	  parlay::sequence<
			   std::pair<indexType, typename Point::distanceType>>,
                    std::pair<
			      parlay::sequence<std::pair<indexType, typename Point::distanceType>>,
			      parlay::sequence<double>>>,
          size_t>
filtered_beam_search_timestamp(
			       const GT &G,
			       const Point p,
  ...
  std::vector<double> visited_timestamp;
  visited_timestamp.reserve(2 * beamSize);
  ...
  while (remain > offset && num_visited < QP.limit) {
    ...
    visited_timestamp.push_back(((double) now.tv_sec) + ((double) now.tv_usec)/1000000.);
  ...
  return std::make_pair(std::make_pair(parlay::to_sequence(frontier),
                                       std::make_pair(parlay::to_sequence(not_sorted_visited), parlay::to_sequence(visited_timestamp))),
                        full_dist_cmps);
}

  
template<typename indexType, typename Point>
struct beamSearchInfo {
  // history of candidate list (frontier history), like when it visited the ith node, what does the frontier look like
  parlay::sequence<parlay::sequence<std::pair<indexType, typename Point::distanceType>>> frontier_history;
  parlay::sequence<std::pair<indexType, typename Point::distanceType>> final_frontier;
  // visited node in the order they were visitede
  parlay::sequence<std::pair<indexType, typename Point::distanceType>> not_sorted_visited;
  // visited node timestamp
  parlay::sequence<double> visited_timestamp;
};

template<typename indexType, typename Point, typename PointRange,
         typename QPoint, typename QPointRange, class GT>
std::pair<beamSearchInfo<indexType, Point>, size_t>
filtered_beam_search_timestamp_and_candidate_list(
			       const GT &G,
			       const Point p,
			       const PointRange &Points,
			       const QPoint qp, const QPointRange &Q_Points,
  ...
  std::vector<std::vector<id_dist>> frontier_history;
  ...
  while (remain > offset && num_visited < QP.limit) {
    ...
    visited.insert(position, current);
    not_sorted_visited.push_back(current);
    frontier_history.push_back(frontier);
  ...
  beamSearchInfo<indexType, Point> beam_search_info;
  beam_search_info.frontier_history = parlay::tabulate(frontier_history.size(), [&] (long query_index) {
    return parlay::to_sequence(frontier_history[query_index]);
  });
  beam_search_info.final_frontier = parlay::to_sequence(frontier);
  beam_search_info.not_sorted_visited = parlay::to_sequence(not_sorted_visited);
  beam_search_info.visited_timestamp = parlay::to_sequence(visited_timestamp);  

  return std::make_pair(beam_search_info,full_dist_cmps);
}
...
std::pair<
	  std::pair<
		    ...
		    std::pair<
			      ...
			      parlay::sequence<double>>>,
          size_t>
beam_search_timestamp(const Point p, const Graph<indexType> &G, const PointRange &Points,
            const parlay::sequence<indexType> starting_points, const QueryParams &QP) {
  return filtered_beam_search_timestamp(G, p, Points, p, Points, starting_points, QP, false);
}


template<typename Point, typename PointRange, typename indexType>  
std::pair<beamSearchInfo<indexType, Point>, size_t>
beam_search_timestamp_and_candidate_list(const Point p, const Graph<indexType> &G, const PointRange &Points,const parlay::sequence<indexType> starting_points, const QueryParams &QP) {
  
  return filtered_beam_search_timestamp_and_candidate_list(G, p, Points, p, Points, starting_points, QP, false);
}
